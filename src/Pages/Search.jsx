        ...
        const items = renderData[cid].map((v) => {
          return (
            <div tabIndex="0" onKeyPress={(e)=>{e.key === 'Enter' && handleClick(e, v.id, false);}} onClick={(e)=>{handleClick(e, v.id, false)}} className={styles.itemContainer} key={ulid()}>
              <img className={styles.image} src={v.imageURL} alt={v.shortName}/>
              <div className={styles.text}>
                <div>{v.title}</div>
                {v.videoURL !== "" &&
                    <img className={styles.playSign} src="https://live.staticflickr.com/65535/51134657326_bcc7161e10_b.jpg" alt="video"/>
                }
              </div>
            </div>
          );
